...
VotingPredict_Batch5 <- vector("list", length = length(RandTotal))


for(i in 801:RandTotal) {
  set.seed(i)
  gc()
...
#        print(paste("Problematic Matching...randseed=",i,",b_idx=",b_idx,sep=""))
  ...
  VotingPredict_Batch5[[i]] = unionPred
}


EID = data$EnterpriseID 
save(VotingPredict_Batch5, unionPair, EID, VotingOutDIR, file=paste(VotingOutDIR, "AggregateVoting_V7F1_Randseed1000_VotingPredict_Batch5.RData",sep=""))
