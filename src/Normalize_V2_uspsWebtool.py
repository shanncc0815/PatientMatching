...
def main():
  ...
  parser = argparse.ArgumentParser(description='Normalize V2: use usps webtool to normalize Address',
  ...
  print_separator("Normalize V2: use usps webtool to normalize Address")
...
#    print(input_df['Zip4Len'].value_counts(sort=True))
    ...
    for i in range(args.start_idx, args.end_idx):
      ...
      xmlstring = "<AddressValidateRequest USERID=\"943NARET1574\"><Address ID=\"%s\"><Address1></Address1><Address2>%s</Address2><City>%s</City><State>%s</State><Zip5>%s</Zip5>%s<Zip4></Zip4></Address></AddressValidateRequest>" % (input_df.loc[i,'EnterpriseID'], input_df.loc[i,'ADDRESS1'], input_df.loc[i,'CITY'], input_df.loc[i,'STATE'], input_df.loc[i,'Zip5'], input_df.loc[i,'Zip4'])
      ...
      #result=re.search('(?is)<Address1[^>]*>(.+?)</Address1>', r.text)
      #MinorAddress = result.group(1) if (result != None) else  ""
      MinorAddress = ""
      ...
      CleanAddress = "%s,%s,%s,%s,%s%s" % (MainAddress, MinorAddress, City, State, Zip5, Zip4)
      with open("%s/%s.address.txt" % (args.output_directory, input_df.loc[i,'EnterpriseID']) , "w") as text_file1:
        text_file1.write(CleanAddress)

      with open("%s/%s.inp.txt" % (args.output_directory, input_df.loc[i,'EnterpriseID']) , "w") as text_file2:
        text_file2.write(url)

      with open("%s/%s.out.txt" % (args.output_directory, input_df.loc[i,'EnterpriseID']) , "w") as text_file3:
