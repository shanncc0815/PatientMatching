...
write(Out, file = "submit002_Vote100_V3F1.csv", append = FALSE, sep = " ")

submit = strsplit(unionPair[which(Vote > 0)], split="_")
Out = paste(data$EnterpriseID[as.numeric(sapply(submit,function(X) X[1]))], data$EnterpriseID[as.numeric(sapply(submit,function(X) X[2]))],1,sep=",")
#write(Out, file = "submit001_Vote3.csv", append = FALSE, sep = " ")
write(Out, file = "submit003_Vote100g0_V3F1.csv", append = FALSE, sep = " ")

submit = strsplit(unionPair[which(Vote == 0)], split="_")
Out = paste(data$EnterpriseID[as.numeric(sapply(submit,function(X) X[1]))], data$EnterpriseID[as.numeric(sapply(submit,function(X) X[2]))],1,sep=",")
#write(Out, file = "submit001_Vote3.csv", append = FALSE, sep = " ")
write(Out, file = "unsubmit_Vote100unlink_V3F1.csv", append = FALSE, sep = " ")
rm(submit)
rm(Out)
gc()

save.image(file=paste(VotingOutDIR, "AggregateVoting_V3F1.RData",sep=""))
