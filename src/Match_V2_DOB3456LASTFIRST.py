...
#    result_df.to_csv('meta3/sameDOB3.csv', index=False)

    idx_4 = np.where(FREQ == 4)[0]
    for i in range(len(idx_4)):
      thisDOB = DOBs[idx_4[i]]
      idx_DOB = np.where(input_df['DOB'] == thisDOB)[0]
      if i == 0:
        result_df = input_df.iloc[idx_DOB,]
      else:
        result_df = pd.concat([result_df, input_df.iloc[idx_DOB,]])
#    result_df.to_csv('meta3/sameDOB4.csv', index=False)

    idx_5 = np.where(FREQ == 5)[0]
    for i in range(len(idx_5)):
      thisDOB = DOBs[idx_5[i]]
      idx_DOB = np.where(input_df['DOB'] == thisDOB)[0]
      if i == 0:
        result_df = input_df.iloc[idx_DOB,]
      else:
        result_df = pd.concat([result_df, input_df.iloc[idx_DOB,]])
#    result_df.to_csv('meta3/sameDOB5.csv', index=False)

    idx_6 = np.where(FREQ == 6)[0]
    for i in range(len(idx_6)):
      thisDOB = DOBs[idx_6[i]]
      idx_DOB = np.where(input_df['DOB'] == thisDOB)[0]
      if i == 0:
        result_df = input_df.iloc[idx_DOB,]
      else:
        result_df = pd.concat([result_df, input_df.iloc[idx_DOB,]])
    result_df.to_csv('meta3/sameDOB6.csv', index=False)
