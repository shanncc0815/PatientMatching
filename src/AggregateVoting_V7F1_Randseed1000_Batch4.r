...
VotingPredict_Batch4 <- vector("list", length = length(RandTotal))


for(i in 601:RandTotal) {
  set.seed(i)
  gc()
...
#        print(paste("Problematic Matching...randseed=",i,",b_idx=",b_idx,sep=""))
  ...
  VotingPredict_Batch4[[i]] = unionPred
}


EID = data$EnterpriseID 
save(VotingPredict_Batch4, unionPair, EID, VotingOutDIR, file=paste(VotingOutDIR, "AggregateVoting_V7F1_Randseed1000_VotingPredict_Batch4.RData",sep=""))
