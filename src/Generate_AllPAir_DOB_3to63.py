...
def main():
  ...
  parser = argparse.ArgumentParser(description='Generate_AllPAir_DOB_3to63', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  ...
  parser.add_argument('output_directory', metavar='output-directory', default='DOBcsv', type=str,
                        help='Directory for merged data for furthur examination.')
  args = parser.parse_args()

  print_separator("Generate all possible pairs of patients with the same DOB")
...
#          print("%s,%s,1" % (result_df.iloc[j,0],result_df.iloc[k,0]))


    for N in range(min(FREQ), max(FREQ)+1):
      idx = np.where(FREQ == N)[0]
      print_separator('Frequency = %d : %d DOB' % (N, len(idx)))
      if len(idx) > 0:
        output_fname = "%s/sameDOB_FREQ%02d.csv" % (args.output_directory, N)
        count = 0;
        with open(output_fname, 'w') as DOBcsv:
          for i in range(len(idx)):
            thisDOB = DOBs[idx[i]]
            idx_DOB = np.where(input_df['DOB'] == thisDOB)[0]
            result_df = input_df.iloc[idx_DOB,]
        ...
        print("No output file was created because no DOB with Frequency = %d" % N)
